...
**PythonからTypeScriptへの移行。** フックスクリプトは将来のリリースでTypeScriptに移行予定です。この移行は、まだ公開していない他の作業と連動しているため、自分自身で進めます。この移行を提案するPRは、感謝の言葉とともにクローズさせていただきます。

**その他のコントリビューションは歓迎です。** 1ファイルを超える変更については、まずIssueを開いてください。特にフック、インストール手順、必要な環境に関わる変更については必須です。マージできないものを作る手間を省けます。
